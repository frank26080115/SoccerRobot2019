...
static inline void handle_interrupts(timer16_Sequence_t timer, volatile uint16_t *TCNTn, volatile uint16_t* OCRnA)
{
  if ( Channel[timer] < 0 )
    *TCNTn = 0; // channel set to -1 indicated that refresh interval completed so reset the timer
    ...
    if ( SERVO_INDEX(timer,Channel[timer]) < ServoCount && SERVO(timer,Channel[timer]).Pin.isActive != false )
  ...
  {
    uint32_t tcntn = *TCNTn;
    uint32_t targetTicks = SERVO(timer,Channel[timer]).ticks;
    uint32_t curTicks = SERVO(timer,Channel[timer]).oldticks;
    int32_t accel = SERVO(timer,Channel[timer]).accel;
    uint32_t nt;
    ...
    SERVO(timer,Channel[timer]).oldticks = curTicks;
    nt = tcntn + curTicks;
    nt /= SERVO_ACCEL_SCALE;
    *OCRnA = (uint16_t)nt;
    if(SERVO(timer,Channel[timer]).Pin.isActive != false) {     // check if activated
...
uint8_t ContinuousServo::attach(int pin)
{
  return this->attach(pin, MIN_PULSE_WIDTH, MAX_PULSE_WIDTH);
}

uint8_t ContinuousServo::attach(int pin, uint16_t min, uint16_t max)
{
  if(this->servoIndex < MAX_SERVOS ) {
    pinMode(pin, OUTPUT) ;                                   // set servo pin to output
    servos[this->servoIndex].Pin.nbr = pin;
	servos[this->servoIndex].accel = 0;
...
void ContinuousServo::writeMicroseconds(int value)
{
  ...
  if( (channel < MAX_SERVOS) )   // ensure channel is valid
  {
    ...
    cli();
    servos[channel].ticks = value * SERVO_ACCEL_SCALE;
    if (servos[channel].oldticks < ((SERVO_MIN() / 2) * SERVO_ACCEL_SCALE))
    {
      servos[channel].oldticks = servos[channel].ticks;
    }
    SREG = oldSREG;
  }
}

void ContinuousServo::writeTicks(uint16_t value)
{
  // calculate and store the values for the given channel
  byte channel = this->servoIndex;
  if( (channel < MAX_SERVOS) )   // ensure channel is valid
  {
    value -= usToTicks(TRIM_DURATION);
    uint8_t oldSREG = SREG;
    cli();
    servos[channel].ticks = value * SERVO_ACCEL_SCALE;
    if (servos[channel].oldticks < ((SERVO_MIN() / 2) * SERVO_ACCEL_SCALE))
...
void ContinuousServo::setAccel(int value)
{
  // calculate and store the values for the given channel
  byte channel = this->servoIndex;
  if( (channel < MAX_SERVOS) )   // ensure channel is valid
  {
    uint8_t oldSREG = SREG;
    cli();
    servos[channel].accel = value;
    SREG = oldSREG;
  }
}

uint32_t ContinuousServo::getTicks(void)
{
	byte channel = this->servoIndex;
	if( (channel < MAX_SERVOS) )   // ensure channel is valid
	{
		uint32_t v = servos[channel].ticks / SERVO_ACCEL_SCALE;
		v += usToTicks(TRIM_DURATION);
		return v;
	}
	return 0;
}

int ContinuousServo::read() // return the value as degrees
{
  return  map( this->readMicroseconds()+1, SERVO_MIN(), SERVO_MAX(), 0, 180);
}

int ContinuousServo::readMicroseconds()
{
...
uint8_t ContinuousServo::attach(int pin, uint16_t minimum, uint16_t maximum)
{
...
uint8_t ContinuousServo::attach(int pin, uint16_t minUs, uint16_t maxUs)
{
  if (!_attached) {
...
uint32_t ContinuousServo::getTicks()
{
